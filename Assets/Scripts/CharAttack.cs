...
public class CharAttack : StateMachineBehaviour {
	...
	override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex) {
		...
		if(weapon.weaponType == WeaponManager.WeaponType.Melee){
			fists = player.GetComponentInChildren<WeaponControl> ().other;
			try//if there is any alien coliding with the fists
			{
				fists.GetComponentInParent<EnemyAlien> ().MeleeHit (100000f);	
			}catch{}
		}
	}
